...
function UploadInvoiceModal({ onClose }: { onClose: () => void }) {
  ...
  // Auto-cerrar modal después de upload exitoso
  useEffect(() => {
    if (uploadResult?.success && !isUploading) {
      const timer = setTimeout(() => {
        onClose();
        window.location.reload(); // Recargar para ver nuevas facturas
      }, 3000);
      return () => clearTimeout(timer);
    }
  }, [uploadResult, isUploading, onClose]);
          ...
          <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700">
            ...
            >
              <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">
                {uploadResult.success ? "✅ Procesamiento Completado" : "❌ Error"}
              </h4>
              {uploadResult.success && (
                <div className="text-sm text-gray-700 dark:text-gray-300 space-y-1">
                  <p>
                    📊 Procesadas: <strong>{uploadResult.processed}</strong>
                  </p>
                  <p>
                    ✅ Auto-aprobadas: <strong>{uploadResult.approved}</strong>
                  </p>
                  <p>
                    ⚠️ Requieren revisión: <strong>{uploadResult.needsReview}</strong>
                  </p>
