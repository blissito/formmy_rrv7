 ...
 */
// Tool execution will be handled via context.tools and custom execution
export class AgentExecutor {
    ...
    async run(context) {
        ...
        while (iterations < maxIterations) {
            ...
            try {
                ...
                const thought = await this.core.executeWithRetry(() => this.think(context, this.memory), `think-iteration-${iterations}`);
                ...
                const action = await this.core.executeWithRetry(() => this.act(thought, context), `act-iteration-${iterations}`);
                ...
                if (iterations === 1) {
                    return {
                        response: this.core.generateUserFriendlyError(error, 'agent-execution'),
                        toolsUsed,
                        iterations,
                        success: false
                    };
     ...
     */
    calculateMaxIterations(message) {
    ...
    estimateComplexity(message) {
        ...
        if (hasToolKeywords)
            return 'high'; // Herramientas siempre son complejas
        if (hasComplexWords && message.length > 50)
            return 'high';
        if (hasComplexWords || message.length > 100)
            return 'medium';
        return 'low';
    }
    /**
     ...
     */
    buildToolExecutionPrompt(context, memory) {
...
- Si no estás seguro, confidence < 0.5`;
     ...
     */
    buildThinkingPrompt(context, memory) {
        let prompt = `CONTEXTO:
...
- Confidence alto (>0.8) para acciones claras, bajo (<0.5) si no estás seguro`;
     ...
     */
    async requestDecision(prompt, context) {
     ...
     */
    makeFallbackDecision(message, tools) {
        const messageLC = message.toLowerCase();
     ...
     */
    generateToolArgs(toolName, message) {
        switch (toolName) {
     ...
     */
    async executeTool(thought, context) {
        const decision = await this.requestDecision(this.buildToolExecutionPrompt(context, this.memory), context);
        ...
        try {
            ...
            const toolResult = await this.executeToolCall(tool.name, decision.args, toolContext, context);
            ...
            return {
                ...
                tool: tool.name,
                input: decision.args,
                content: `Error ejecutando ${tool.name}: ${error.message}`
            };
        }
    }
    /**
     ...
     */
    async generateResponse(thought, context) {
        const providerWrapper = this.createProviderWrapper(context);
...
`;
     ...
     */
    async executeToolCall(toolName, args, toolContext, context) {
        try {
            // Si hay un ejecutor de herramientas personalizado en el contexto
            if (context.toolExecutor && typeof context.toolExecutor === 'function') {
                return await context.toolExecutor(toolName, args, toolContext);
            }
            // Buscar la herramienta en el array de herramientas disponibles
            ...
            // Si la herramienta tiene un ejecutor personalizado
            ...
            // Fallback: simulación de ejecución
            return {
                success: true,
                message: `Herramienta '${toolName}' ejecutada con argumentos: ${JSON.stringify(args)}`
            };
        }
        catch (error) {
            ...
            return {
                success: false,
                message: `Error ejecutando ${toolName}: ${error.message}`
            };
        }
    }
    /**
     ...
     */
    createProviderWrapper(context) {
        return {
            generateResponse: async (prompt, allowStreaming) => {
                // Si hay un proveedor de AI configurado, usarlo
                if (context.aiProvider && typeof context.aiProvider.chatCompletion === 'function') {
                    try {
                        ...
                        const result = await context.aiProvider.chatCompletion(chatRequest);
                ...
                return `Framework funcionando. AI Provider no configurado. Prompt: "${prompt.substring(0, 100)}..."`;
            }
        };
    }
    /**
     ...
     */
    explainReasoning(decision, context) {
    ...
    isTaskComplete(content) {
        ...
        return completionIndicators.some(indicator => content.toLowerCase().includes(indicator));
    }
}
