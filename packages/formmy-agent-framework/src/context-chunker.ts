...
export class ContextChunker {
   ...
   */
  selectRelevant(chunks: ContextChunk[], query: string, limit = 3): ContextChunk[] {
    ...
    for (const context of contexts) {
      ...
      switch (context.type) {
        case 'TEXT':
          ...
          break;
        case 'QUESTION':
          content = `${context.title}\n\nPreguntas: ${(context as any).questions}\n\nRespuesta: ${(context as any).answer}`;
          break;
        case 'FILE':
        case 'LINK':
   ...
   */
  private createChunk(content: string, source?: string): ContextChunk {
    return {
  ...
  private calculateKeywordRelevance(
    ...
    const commonKeywords = chunkKeywords.filter(kw => 
      queryKeywords.some(qkw => 
        qkw.includes(kw) || kw.includes(qkw) || this.levenshteinSimilarity(kw, qkw) > 0.8
      )
    );
