...
`;
   ...
   */
  private async executeToolCall(toolName: string, args: any, toolContext: any, context: AgentContext): Promise<{success: boolean, message: string}> {
    try {
      ...
      return {
        success: false,
        message: `Error ejecutando ${toolName}: ${(error as Error).message}`
      };
    }
  }
