...
const FormmyContext = createContext<FormmyContextValue | null>(null);

export function FormmyProvider({
  publishableKey,
  secretKey,
  ...
  if (publishableKey && !VALID_PK_PREFIXES.some(p => publishableKey.startsWith(p))) {
    console.warn(
      "[FormmyProvider] Invalid publishableKey format. Expected formmy_pk_live_xxx"
    );
  }

  const value = useMemo<FormmyContextValue>(
    () => ({
      publishableKey,
      secretKey,
      baseUrl,
  ...
  return (
    <FormmyContext.Provider value={value}>{children}</FormmyContext.Provider>
  );
}
 ...
 */
export function useFormmy(): FormmyContextValue {
 ...
 */
export function useFormmyOptional(): FormmyContextValue | null {
