 ...
 * ```
 */
export function createFormmyTool(config) {
        ...
        get definition() {
            ...
            if (!tool || !z) {
                try {
                    ...
                    throw new Error('LlamaIndex integration requires "llamaindex" and "zod" packages. ' +
            ...
            return tool({
                ...
                handler: async ({ query }) => {
                    try {
                        const result = await config.client.query(query, config.chatbotId, {
 ...
 * ```
 */
export function createMultipleFormmyTools(client, configs) {
