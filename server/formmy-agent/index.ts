...
import { getAvailableTools as getRegistryTools } from '../tools/registry';
...
export class FormmyAgent {
  ...
  private async getAvailableTools(options: ChatOptions): Promise<any[]> {
    try {
      ...
      // Usar el registry real de herramientas
      const userPlan = options.user.plan || 'FREE';
      const integrations = {}; // TODO: pasar integraciones reales si están disponibles
      const includeDescriptions = true;
      
      return getRegistryTools(userPlan, integrations, includeDescriptions);
    } catch (error) {
...
Responde de forma directa y útil basándote en el contexto proporcionado.`;
  ...
  private async generateSimpleResponse(prompt: string, context: AgentContext): Promise<string> {
    return await this.core.executeWithRetry(async () => {
      // Importar y usar el sistema de proveedores real
