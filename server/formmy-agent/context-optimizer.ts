...
export class ContextOptimizer {
   ...
   */
  async optimize(contexts: ContextItem[], userMessage: string): Promise<string> {
    ...
    const relevantChunks = this.chunker.selectRelevant(
      allChunks, 
      userMessage, 
      Math.min(8, allChunks.length) // Máximo 8 chunks para evaluar
    );
   ...
   */
  private formatChunk(content: string, source?: string): string {
    if (source) {
      return `## ${source}\n\n${content}\n\n`;
    }
    return `${content}\n\n`;
   ...
   */
  private addContextSummary(context: string, userMessage: string): string {
...
${context}`;
   ...
   */
  private generateContextSummary(context: string, userMessage: string): string {
    ...
    const relevantKeywords = contextKeywords
      .filter(kw => queryKeywords.some(qkw => 
  ...
  private calculateContextRelevance(
    ...
    // Palabras en común
    const commonWords = contextKeywords.filter(kw => 
